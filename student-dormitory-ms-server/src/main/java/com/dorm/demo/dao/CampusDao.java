...
public interface CampusDao extends JpaRepository<CampusInfo, Integer> {
    ...
    @Query(nativeQuery=true,value = "" +
            ...
            "(select @row1\\:=@row1 + 1 as row_number ,t.* from (select campus.id,campus.name,COUNT(campus.id) as dorm_num from dorm left join campus on dorm.campus_id=campus.id group by campus.id) t ,(select @row1\\:=0) r) a " +
            "left join " +
            "(select @row2\\:=@row2 + 1 as row_number ,t.* from (select COUNT(dorm_campus_id) as room_num from room  group by dorm_campus_id) t ,(select @row2\\:=0) r) b " +
            "on a.row_number=b.row_number) c " +
            "left join " +

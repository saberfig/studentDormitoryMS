...
public interface RoomDao extends JpaRepository<RoomInfo, Integer> {
    ...
    @Query(nativeQuery = true, value = "" +
            "select c.dorm_name,campus.name as campus_name,c.dorm_id,c.campus_id,c.room_id,c.bed_num,campus.id as name from (select a.bed_num,a.dorm_id,a.campus_id,a.room_id,dorm.name as dorm_name from (select count(*) as bed_num,room_id,room_dorm_id as dorm_id,room_dorm_campus_id as campus_id from bed group by room_id,bed.room_dorm_id,room_dorm_campus_id) as a left join dorm on a.dorm_id = dorm.id and a.campus_id = dorm.campus_id)as c left join campus on c.campus_id = campus.id")
    List<RoomInfo> getRoomInfo();
}